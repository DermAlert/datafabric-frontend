    ...
    datasets: [
      {
        ...
        layer: 'silver',
        rowCount: 125430,
        description: 'Normalized patient data with CPF formatting',
      },
      {
        id: 'ds_2',
        name: 'patients_raw',
        layer: 'bronze',
        rowCount: 234567,
        description: 'Raw patient data from healthcare sources',
      },
    ],
    recipients: [
    ...
    datasets: [
      {
        ...
        layer: 'bronze',
        rowCount: 1234567,
        description: 'Unified orders from all channels',
      },
    ],
    recipients: [{ id: 'rec_3', name: 'BI Team', email: 'bi@company.com' }],
  },
  {
    id: 'share_3',
    name: 'inventory_data',
...
const ShareListItem = memo(function ShareListItem({
  share,
  isSelected,
  onSelect,
}) {
...
const DatasetCard = memo(function DatasetCard({ dataset, onRemove }) {
  return (
    ...
    <div className="p-4 border border-gray-200 dark:border-zinc-700 rounded-lg hover:border-violet-200 dark:hover:border-violet-800 transition-colors">
      ...
      {fullRecipient && (
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-zinc-800 flex items-center justify-between">
          ...
          <button
            onClick={() => onCopyToken(recipient.id, 'full-token-here')}
...
const DetailPanel = memo(function DetailPanel({
  ...
  const getTotalRows = () => share.datasets.reduce((acc, ds) => acc + ds.rowCount, 0);
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{share.description}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex">
          ...
          >
            <span className="flex items-center gap-2">
              ...
              Shared Datasets ({share.datasets.length})
            </span>
          </button>
          <button
      ...
      <div className="p-6">
        {activeTab === 'schemas' && (
          <div className="space-y-4">
            ...
            {share.datasets.length === 0 ? (
              <EmptyState
                icon={<Database className="w-12 h-12" />}
                title="No datasets shared yet"
                description="Add Bronze or Silver datasets to this share"
                action={
                  <button
                    onClick={onAddDataset}
              ...
              <div className="space-y-2">
                {share.datasets.map((dataset) => (
                  <DatasetCard key={dataset.id} dataset={dataset} onRemove={onRemoveDataset} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {share.recipients.length === 0 ? (
              <EmptyState
                ...
                description="Add recipients to share this data"
                action={
                  <button
                    onClick={onAddRecipient}
                    className="mt-4 px-4 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors text-sm font-medium inline-flex items-center gap-2"
              ...
              <div className="space-y-3">
                {share.recipients.map((recipient) => {
                  const fullRecipient = recipients.find((r) => r.id === recipient.id);
                  return (
                    <RecipientCard
                      key={recipient.id}
                      recipient={recipient}
                      fullRecipient={fullRecipient}
      ...
      <footer className="px-6 py-4 border-t border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/50">
        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>Last updated {formatDate(share.updatedAt)}</span>
        </div>
      </footer>
    </div>
  );
...
const CreateShareModal = memo(function CreateShareModal({
  isOpen,
  onClose,
  onCreate,
}) {
...
const AddDatasetModal = memo(function AddDatasetModal({ isOpen, onClose }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Dataset to Share">
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
      ...
      <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <div className="flex items-start gap-2">
          ...
          <p className="text-sm text-blue-700 dark:text-blue-300">
            Only <strong>Persistent</strong> datasets can be shared via Delta Sharing. Virtualized
            datasets query data on-demand and are not stored in Delta Lake.
          </p>
        </div>
      </div>
...
const AddRecipientModal = memo(function AddRecipientModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Recipient">
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Grant access to this share</p>
      ...
      <div className="space-y-3">
        ...
        {availableRecipients.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">
            All recipients already have access to this share
          </p>
        ) : (
...
export default function SharingPage() {
  ...
  // Available recipients (not already in selected share)
  const availableRecipients = MOCK_RECIPIENTS.filter(
    (r) => !selectedShare?.recipients.find((sr) => sr.id === r.id)
  );
  ...
  const handleCopyToken = useCallback((recipientId, token) => {
  ...
  const handleRemoveRecipient = useCallback((id) => {
    console.log('Remove recipient:', id);
  }, []);

  const handleCreateShare = useCallback((data) => {
    console.log('Create share:', data);
    createShareModal.onClose();
  }, [createShareModal]);

  // Stats
  const stats = {
    shares: shares.length,
    recipients: MOCK_RECIPIENTS.length,
    datasets: shares.reduce((acc, s) => acc + s.datasets.length, 0),
  };
  ...
  return (
    <DashboardLayout>
      <div className="p-8">
        ...
        <header className="mb-8">
          ...
          <div className="flex items-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
              <Users className="w-4 h-4" />
              <span>{stats.recipients} recipients</span>
            </div>
            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
              <Table2 className="w-4 h-4" />
              <span>{stats.datasets} shared datasets</span>
            </div>
          </div>
        </header>
        ...
        <div className="flex gap-6">
          ...
          <main className="flex-1">
            <DetailPanel
              ...
              copiedToken={copiedToken}
              onAddDataset={addDatasetModal.onOpen}
      ...
      <AddDatasetModal isOpen={addDatasetModal.isOpen} onClose={addDatasetModal.onClose} />
